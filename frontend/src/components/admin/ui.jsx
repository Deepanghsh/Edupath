...
export function ScoreBar({ val, max = 100 }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
      <div style={{ width: 64, height: 5, background: C.gray200, flexShrink: 0 }}>
...
export function Btn({ variant = 'ghost', size = '', onClick, children }) {
  const base = { display: 'inline-flex', alignItems: 'center', gap: 4, cursor: 'pointer', border: '1px solid', fontFamily: 'IBM Plex Sans, sans-serif', letterSpacing: '0.2px', lineHeight: 1.5, whiteSpace: 'nowrap', fontWeight: 500 };
  const sizes = { '': { padding: '4px 11px', fontSize: 11 }, sm: { padding: '3px 8px', fontSize: 10.5 }, lg: { padding: '8px 18px', fontSize: 13 } };
  const variants = {
    primary: { background: C.accent,    color: '#fff',     borderColor: C.accent },
    ghost:   { background: '#fff',      color: C.gray600,  borderColor: C.gray200 },
    success: { background: C.successBg, color: C.success,  borderColor: '#9fcfb4' },
    danger:  { background: C.dangerBg,  color: C.danger,   borderColor: '#e8b4b4' },
    gold:    { background: C.gold,      color: '#fff',     borderColor: C.gold },
    outline: { background: '#fff',      color: C.accent,   borderColor: C.accent },
  };
  ...
  return <button onClick={handleClick} style={{ ...base, ...sizes[size], ...variants[variant] }}>{children}</button>;
}
...
export function KpiCard({ label, value, sub, accent = C.accent }) {
  return (
    <div style={{ background: '#fff', border: `1px solid ${C.gray200}`, padding: '16px 18px', borderTop: `3px solid ${accent}` }}>
...
export function TableCard({ children, style }) {
  return <div style={{ background: '#fff', border: `1px solid ${C.gray200}`, marginBottom: 22, ...style }}>{children}</div>;
}
